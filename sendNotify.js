...
function qywxamNotify(text, desp) {
  return new Promise(resolve => {
    if (QYWX_AM) {
        ...
        $.post(options, (err, resp, data) => {
          try {
            if (err) {
              ...
              data = JSON.parse(data);
              if (data.errcode === 0) {
                console.log('成员ID:' + ChangeUserId(desp) + '企业微信应用消息发送通知消息成功ߎ飀✮');
              } else {
                console.log(`${data.errmsg}\n`);
              }
            }
      ...
      console.log('您未提供企业微信应用消息推送所需的QYWX_AM，取消企业微信应用消息推送消息通知ߚ뜮');
      resolve();
    }
  });
}
...
function iGotNotify(text, desp, params={}){
  return  new Promise(resolve => {
    if (IGOT_PUSH_KEY) {
      ...
      console.log('您未提供iGot的推送IGOT_PUSH_KEY，取消iGot推送消息通知ߚ뜮');
      resolve()
    }
  })
}
...
function pushPlusNotify(text, desp) {
  return new Promise(resolve => {
    if (PUSH_PLUS_TOKEN) {
      ...
      console.log('您未提供push+推送所需的PUSH_PLUS_TOKEN，取消push+推送消息通知ߚ뜮');
      resolve()
    }
  })
}
